...
async function runMigration() {
    ...
    if (!MONGODB_URI || MONGODB_URI.includes("<") || MONGODB_URI.includes(">")) {
        console.error("[Migration] ERROR: MongoDB connection string is missing, invalid, or contains a placeholder password.");
        console.error("[Migration] Please set the MONGODB_URI environment variable with your actual connection string.");
        process.exit(1);
    }
    ...
    try {
        ...
        // --- Perform the Update ---
        console.log("[Migration] Finding users missing default fields (admin, profilePicture, aboutMe)...");
        ...
        console.log(`[Migration] Update complete. Matched ${result.matchedCount} users and modified ${result.modifiedCount} users.`);
        if (result.matchedCount > 0 && result.modifiedCount === 0) {
             console.log("[Migration] Note: Users found already had the fields, no modification needed.");
