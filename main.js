...
// --- IPC Handlers ---
function sendToServer(messageObject) { if (clientSocket && clientSocket.readyState === WebSocket.OPEN) { clientSocket.send(JSON.stringify(messageObject)); return true; } else { console.log('[Client] Cannot send message: Not connected to server.'); return false; } }
...
ipcMain.on('get-all-users', (event) => { if (loggedInUsername) { sendToServer({ type: 'get-all-users' }); } });
...
ipcMain.on('show-sidebar-context-menu', (event) => { if (!isAdmin) return; const template = [ { label: 'Create Channel', click: () => { mainWindow?.webContents.send('prompt-create-channel'); } } ]; const menu = Menu.buildFromTemplate(template); menu.popup({ window: mainWindow }); });
ipcMain.on('show-channel-context-menu', (event, channelName) => { if (!isAdmin || channelName === 'general') return; const template = [ { label: `Delete #${channelName}`, click: () => { mainWindow?.webContents.send('confirm-delete-channel', channelName); } } ]; const menu = Menu.buildFromTemplate(template); menu.popup({ window: mainWindow }); });
ipcMain.on('show-message-context-menu', (event, { messageId, isOwnMessage }) => { const template = []; if (isOwnMessage) { template.push( { label: 'Edit Message', click: () => { mainWindow?.webContents.send('edit-message-prompt', messageId); } }, { type: 'separator' }, { label: 'Delete Message', click: () => { sendToServer({ type: 'delete-message', messageId: messageId }); } } ); } else { template.push({ label: 'Copy Message ID (soon)', enabled: false }); } if (template.length > 0) { const menu = Menu.buildFromTemplate(template); menu.popup({ window: mainWindow }); } });
ipcMain.on('request-status', (event) => { event.reply('status-update', { connected: !!loggedInUsername, wsConnected: clientSocket && clientSocket.readyState === WebSocket.OPEN, connecting: clientSocket && clientSocket.readyState === WebSocket.CONNECTING, searching: false, serverIp: serverAddress, localHostname: os.hostname(), username: loggedInUsername, currentChannel: currentChannel, isAdmin: isAdmin }); });
